...
| 無音判定閾値 | セッション限りの値。**config には保存されない**（マイク・会場依存のため）。毎回「無音測定」で設定し、その値はメモリで本体に渡る |
| トリガ間隔 | `settings.cooldown_seconds` |
| 無音測定マージン (dB) | `settings.launcher.silence_margin_db`（無音測定の式にのみ影響） |
| 無音測定ボタン / NCチェックボタン | （下記） |
| 特徴量・確信度モニタを開く | `--viz` |
| 詳細ログ | `-v` |

各項目の右の ⓘ（または項目名）にマウスを載せると、その設定の説明がポップアップで表示される（ⓘ はクリックでも開閉できる）。

**無音測定ボタン**: ステージが無音（暗騒音のみ）の状態で測定し、無音判定閾値を自動設定する。
閾値は使用するマイクと会場の暗騒音に依存するため **config には保存されない**（セッション限り）。
別の会場・マイクで作った config を使い回しても不適切な閾値が黙って適用されることはない代わりに、
**本番のたびにこのボタンで測定する**運用が前提。CLI 起動（config を引数指定）では測定 UI が無いので
既定値 `-55.0 dBFS` で始まり、実行時に ↑/↓・±ボタンで調整する。
...
- 「開始」で選択内容が**選択した config.json に保存**され（ランチャー専用の状態は
  `settings.launcher` ブロック、デバイスや `cooldown_seconds` は既存のフラットキー）、
  次回起動時に復元される。**無音判定閾値だけは保存されない**（マイク・会場依存のセッション値で、
  メモリで本体へ渡る。既存 config に残った旧 `silence_threshold_db` は保存時に除去される）
...
| ~~`silence_threshold_db`~~ | — | **config には保存されない**（マイク・会場依存のため意図的に非連動）。無音判定閾値はランチャーの「無音測定」でセッション限りに設定し、メモリで本体へ渡る。既定値は `-55.0 dBFS`。この dBFS 以下が続くと OLTW を一時停止（フェルマータ・休符対策）。`--input-wav` / `--loopback` モードでは gate 自体が自動無効。ホール本番は観客の咳・空調で暗騒音が **-50 〜 -45 dBFS** 程度まで上がるため、本番のたびにステージ無人時のレベルを「無音測定」で実測すること。 |
...
起動すると「Audio Score Follower 操作コンソール」が開く。1200x800 の画面に収まる
サイズで、情報はカード単位にまとめてある：

| カード | 内容 |
|---|---|
| 曲 | `1曲目 / 全3曲` と `次の曲: 2曲目（N キーで移動）`。config の `movements` が「曲」として並ぶ |
| 現在位置 | 現在小節（大）/ 全小節数、拍 |
| 追随の確信度 | 表示確信度（下記） |
| トリガー | `前: N 小節目` / `次: N 小節目`。「前」は現在位置以前で最後のトリガー小節（まだ通過していなければ 1 = 冒頭のスライド）。クールダウン中はその下に表示 |
| マイク | 入力レベル dBFS + 無音判定閾値と −/＋ ボタン |
| 追随モード | 待機 / 追随中 / 慣性 / 慣性停止 + 「▶ 演奏開始」「■ 演奏終了」ボタン |
...
| **N** | 次の曲をロード（config の `movements` の次の要素） |
| **R** | 現在の曲を再ロード (OLTW reset)。スライドも 1 枚目に戻る（measure 1 のトリガーで再び 2 枚目へ進む） |
...
| **E** / 「■ 演奏終了」ボタン | **演奏終了の指示**（Issue #44）。演奏が終わっても follower は追随を続け、拍手・環境音で小節が進みトリガーが出てしまう。押下で OLTW ワーカーを止めてフレーム供給を絶ち、追随を停止 + 自動トリガーを抑止する。当該曲に対して終端的で、再び追随するには **R（再ロード）/ N（次の曲）** を押す。ボタンは演奏進行中のみ表示 |
...
（成立後は押しても効果がないため自動非表示。曲の再ロードで再表示）、「■ 演奏終了」
ボタンは演奏進行中のみ表示される。

**状態の呼び方と判定は聴衆画面と共通**（`ui/audience_panel.resolve_status` が正本）。
操作コンソールがピットで「追随中」と表示している隣で、客席のパネルが「見失い中」を
出している——という食い違いを作らないため、**語と色は聴衆画面に揃え、操作者にしか
要らない情報（復帰キー・慣性の残り秒数）だけを括弧で足す**：

| 状態（両画面共通） | 操作コンソールの表示 | 色 | 判定 |
|---|---|---|---|
| 待機中 | `⏸ 待機中（▶ 演奏開始 を押してください）` | グレー | 手動スタート待ち |
| 待機中 | `⏹ 待機中（演奏終了 — R で再追随 / N で次の曲）` | グレー | 「■ 演奏終了」/ E で停止（Issue #44） |
| 曲を捕捉中 | `🎧 曲を捕捉中（無音でも 3 秒後に自動開始）` | 琥珀 | スタート押下後、演奏確定前（gate 開放 or 見切りタイムアウト待ち） |
| 曲を捕捉中 | `🎧 曲を捕捉中` | 琥珀 | 演奏確定後、lock-in 前 |
| 追随中 | `🎵 追随中` | 緑 | lock-in 済み・表示確信度 60% 超 |
| 確認中 | `🔎 確認中（確信度が低下しています）` | 橙 | 表示確信度 40〜60%、または 40% 以下が 3 秒未満 |
| 見失い中 | `🌀 見失い中（慣性進行中　残り 6.3s / 10s — 音が戻れば自動復帰）` | 赤 | 慣性モード |
| 見失い中 | `⛔ 見失い中（慣性停止・位置固定）　手動 → / L で復帰してください` | 赤 | 慣性 cap 到達 |
| 見失い中 | `⚠ 見失い中（追随ずれ疑い — ←/→ で補正可）` | 赤 | mismatch detector が発火 |
| 見失い中 | `⚠ 見失い中（確信度が低いまま — ←/→ で補正可）` | 赤 | 表示確信度 40% 以下が 3 秒持続 |
| （操作コンソールのみ） | `🎯 開始を受け付けました` | 青 | スタート押下後 1 秒のフラッシュ |

確信度による格下げ（追随中 → 確認中 → 見失い中）の根拠は
[docs/calibration.md](docs/calibration.md#5-追随状態の表示聴衆パネル--操作コンソール共通)。
...
ゲネプロ開始前   USB I/F + マイク経路で「ステージ無人時の dBFS」を実測
                 → ランチャーの「無音測定」で無音判定閾値を決定（config 非保存・毎回測定）
...
`--slide-url` を指定すると、Chromium の 1 ウィンドウに **左: Google スライド / 右: 追随パネル** を並べた聴衆向け画面が開く（Issue #8）。本番解像度 1280x800 では左 1000x800・右 280x800 になり、文字サイズはウィンドウサイズに追随する。

- **外部モニター（プロジェクター）を接続してから起動する。** 起動時に外部モニターを検出し、そこへウィンドウを移して全画面にする。ノート PC の画面は操作コンソールのまま残る。外部モニターが無ければ全画面にしない（ノート PC 1 枚でのリハーサル用）。起動直後の数秒は全画面切替で暗転し、「Esc で全画面を終了」の案内が一瞬出る
- **スライドは起動時に 1 枚目を表示する。** 1 枚目はタイトル等にしておき、演奏が始まって measure 1 のトリガーが発火すると 2 枚目へ進む。曲ごとに config（とデッキ）を分け、ランチャーの「開始」で起動する運用を前提にしている
- **パネルの表示:** 小節・拍、確信度（1 秒ごとに更新。色は操作 GUI と同じ基準）、追随状態（待機中 / 曲を捕捉中 / 追随中 / 確認中 / 見失い中。追随中は確信度でも格下げする → [docs/calibration.md](docs/calibration.md#5-追随状態の表示聴衆パネル--操作コンソール共通)）、手動のスライド操作（→ / ← / Space）の直後に約 1 秒「人が調整！」（演奏開始・終了の操作では出ない）。状態ラベルの上にはベルリオーズの顔が出て、状態と「人が調整！」に合わせて表情と背景色が変わる。順調なとき（待機中 / 曲を捕捉中 / 追随中）は止まっていて、確認中はゆっくり揺れ、見失い中は震え、人が調整！では跳ねる
- **デッキはリンク共有（「リンクを知っている全員が閲覧可」）が必要。** `/present` は iframe に埋め込めないため、指定した URL（`/present` / `/edit` / 公開 URL いずれでも可）から自動で `/embed` に変換して表示する。閲覧権限はデッキ全体に及ぶので、聴衆に見せないスライドにも機密情報を載せないこと
- **スライドは文字と画像のみ（アニメーション非対応前提）。**
- **縦横比:** 16:9 のデッキは 1000x800 の枠で上下に黒帯が出る。枠いっぱいに使うなら Google スライドの「ページ設定」でカスタム 5:4（例: 25 x 20 cm）にする
- 聴衆向け画面のウィンドウはクリックしない（スライド上のクリックはページ送りになる）
...
│   ├── slide_controller.py   # Playwright: 聴衆向け画面（Slides + 追随パネル）

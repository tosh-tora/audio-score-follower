...
- **パネルの表示:** 小節・拍、確信度（1 秒ごとに更新。色は操作 GUI と同じ基準）、追随状態（待機中 / 曲を捕捉中 / 追随中 / 確認中 / 見失い中。追随中は確信度でも格下げする → [docs/calibration.md](docs/calibration.md#5-追随状態の表示聴衆パネル--操作コンソール共通)）、手動のスライド操作（→ / ← / Space）の直後に約 1 秒「人が調整！」（演奏開始・終了の操作では出ない）
